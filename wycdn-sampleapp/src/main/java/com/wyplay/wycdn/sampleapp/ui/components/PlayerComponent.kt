    ...
    fun initializePlayer() {
        player = PlayerBuilder(context)
            ...
            .build().apply {
                ...
                on<PlayerEvent.Error> { event ->
                    val message = "Player Error: ${event.code} (${event.code.value}) - ${event.message}"
                    when (event.code) {
                        PlayerErrorCode.DecodingFailed -> { // Immediately try to resume playback
                            Log.w(TAG, message)
                            load(source)
                            play()
                        }
                        else -> { // Update errorMessage with detailed information
                            Log.e(TAG, message)
                            errorMessage = message
                        }
                    }
                }
                on<SourceEvent.Error> { event ->
                    val message = "Source Error: ${event.code} (${event.code.value}) - ${event.message}"
                    when (event.code) {
                        SourceErrorCode.General,
                        SourceErrorCode.HttpStatusCode,
                        SourceErrorCode.Io -> { // Immediately try to resume playback
                            Log.w(TAG, message)
                            load(source)
                            play()
                        }
